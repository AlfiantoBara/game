...
//kotak rounded
    ...
    setTimeout(rafLoops, 1); // akan memanggil func(deltaTime)
}
...
function rafLoops() {
    ...
    var deltaTime = (now - lastTime) / 1000; // dalam detik
    lastTime = now;

    game.loop = requestAnimationFrame(rafLoops);
    loopFunc(deltaTime); // kirim deltaTime ke gameLoop
}
...
// Ambil spawn berdasarkan map yang aktif
//var spawn = spawnPoints[game.mapNum] || { x: 5, y: 5 }; // default ke (1,5) kalau mapNum tidak ditemukan
//game.charX = spawn.x;
//game.charY = spawn.y;
